...
def reverse_model_run(ticker, X_live, model_version):
    ...
    model_pipeline = load_file(
        f"trained_models/{ticker}/pl_predictions/{version}/reverse_pipeline_model.pkl")
    reverse_pipeline_data_cleaning = load_file(
        f"trained_models/{ticker}/pl_predictions/{version}/reverse_pipeline_data_cleaning.pkl")
...
def standard_analysis_reverse(ticker, model_version, sensitivity_adjustment=0.1):
